...
- [`unmasked-secret-exposure`]({{< ref "unmaskedsecretexposure.md" >}}) — detects derived secrets from `fromJson()` expressions that are not masked.
- [`secret-exfiltration`]({{< ref "secretexfiltration.md" >}}) — detects secrets sent to external services via network commands (`curl`, `wget`, `nc`, etc.).
- [`secret-exposure`]({{< ref "secretexposure.md" >}}) — detects excessive secret exposure via `toJSON(secrets)` or `secrets[dynamic-access]`.
