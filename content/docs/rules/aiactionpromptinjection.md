...
On February 17, 2026, the Cline repository was compromised through exactly this pattern. The workflow had `prompt: ${{ github.event.issue.title }}`, which allowed an attacker to inject arbitrary instructions through an issue title, ultimately leading to the exfiltration of `NPM_RELEASE_TOKEN` and the `cline@2.3.0` supply chain attack.
...
{{< popup_link2 href="https://owasp.org/www-project-top-10-for-large-language-model-applications/" >}}

{{< popup_link2 href="https://docs.anthropic.com/en/docs/build-with-claude/prompt-engineering/prompt-injection" >}}
